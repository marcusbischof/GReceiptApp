angular.module('greceipt.login', ['greceipt.service.auth'])
    .controller('LoginCtrl', function($scope, $rootScope, $state, $ionicHistory, $ionicLoading, authService) {
        ...
        $scope.login = function(user, isValid) {
            if (isValid) {
                ...
                authService.signin(user).then(function(result) {
                    console.log(result);
                    // $scope.loginGReceiptWebService($scope.username);
